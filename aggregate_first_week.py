...
'''
def extract_comp_schedule_first_week_data(file_path, output_file='first_week_data.csv'):
    """
    Load dataset, rename 'Unnamed: 0' to 'Date-Time' if necessary,
    and extract data for the first week of October 2024.
    ...
    first_week_data_list = []
    first_weel_ad_slots_list = []

    # drop no numerical columns as they cannoit be aggregated
    for column in df.columns:
        if df[column].dtype == 'object':
            df = df.drop(column, axis=1)
    new_df = df.drop('ad_slot_price', axis=1)
    ad_slot_df = df[['Date-Time', 'ad_slot_price']].dropna()
    # ad_slot_df['Date-Time'] += -timedelta(minutes=5)
    ...
    #     first_week_data_list.append(first_week_data_day)
    for ii in range(7):
        first_week_data = new_df[(new_df['Date-Time'] >= f'2024-10-0{ii+1}') & (new_df['Date-Time'] < f'2024-10-0{ii+2}')]
        first_week_ad_slots = ad_slot_df[(ad_slot_df['Date-Time'] >= f'2024-10-0{ii+1}') & (ad_slot_df['Date-Time'] < f'2024-10-0{ii+2}')]
        first_week_data = first_week_data.set_index('Date-Time').resample('30Min', closed='right', label='right').mean()
        
        # Forward fill to fill missing values with the last valid observation
        first_week_data = first_week_data.ffill()  # Fill forward

        first_weel_ad_slots_list.append(first_week_ad_slots)
        first_week_data_list.append(first_week_data)
        

    first_week_data_df = pd.concat(first_week_data_list)
    first_week_ad_slot_df = pd.concat(first_weel_ad_slots_list)

    first_week_ad_slot_df.drop('Date-Time', axis=1)

    first_week_data_df.reset_index(drop=True)
    first_week_ad_slot_df.reset_index(drop=True)
    print(len(first_week_data_df))
    print(len(first_week_ad_slot_df))
    first_week_data_df = pd.concat([first_week_data_df, first_week_ad_slot_df],axis=1)
