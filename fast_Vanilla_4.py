...
# Convert x variable to numpy array
x = np.array([xp.var(name=f"x_{i}_{j}", vartype=xp.binary) for i in Movies for j in Time_slots])
x = x.reshape(number_of_movies, number_of_time_slots)
model.addVariable(x)

# Convert y variable to numpy array
y = np.array([xp.var(name=f"y_{i}", vartype=xp.binary) for i in Movies])
model.addVariable(y)

# Convert z0 variable to numpy array
z0 = np.array([xp.var(name=f'z0_{i}_{r}', vartype=xp.binary) for i in Movies for r in Ad_slots_0])
z0 = z0.reshape(number_of_movies, number_of_ad_slots_0)
model.addVariable(z0)

# Convert z1 variable to numpy array
z1 = np.array([xp.var(name=f'z1_{i}_{s}', vartype=xp.binary) for i in Movies for s in Ad_slots_1])
z1 = z1.reshape(number_of_movies, number_of_ad_slots_1)
model.addVariable(z1)

# Convert z2 variable to numpy array
z2 = np.array([xp.var(name=f'z2_{i}_{t}', vartype=xp.binary) for i in Movies for t in Ad_slots_2])
z2 = z2.reshape(number_of_movies, number_of_ad_slots_2)
model.addVariable(z2)

# Convert w variable to numpy array
w = np.array([xp.var(name=f"w_{i}_{j}", vartype=xp.binary) for i in Movies for j in Time_slots])
w = w.reshape(number_of_movies, number_of_time_slots)
model.addVariable(w)

# Convert v variable to numpy array
v = np.array([xp.var(name=f"v_{i}_{j}", vartype=xp.binary) for i in Movies for j in Time_slots])
v = v.reshape(number_of_movies, number_of_time_slots)
model.addVariable(v)

# Convert u variable to numpy array
u = np.array([xp.var(name=f"u_{i}_{j}", vartype=xp.continuous) for i in Movies for j in Time_slots])
u = u.reshape(number_of_movies, number_of_time_slots)
model.addVariable(u)

# Convert start variable to numpy array
start = np.array([xp.var(name=f's_{i}', vartype=xp.continuous) for i in Movies])
model.addVariable(start)

# Convert end variable to numpy array
end = np.array([xp.var(name=f'e_{i}', vartype=xp.continuous) for i in Movies])
