...
def model_3day(budget, movie_db_df, my_channel_df, channel_0_df, channel_1_df, channel_2_df, genre_conversion_0_df, genre_conversion_1_df, genre_conversion_2_df, population):
    ...
    model.addConstraint(
        start[i] <= x[i][j]*(my_channel_df['Date-Time'].iloc[j] - start_of_week).total_seconds()/60 + (1 - x[i][j])*(10080 - movie_db_df['runtime_with_ads'].iloc[i])
        for i in Movies for j in Time_slots
        )
...
if __name__ == "__main__":
    ...
    genre_conversion_0_df = pd.read_csv('data/movie_adslots_conversion_0_3block.csv')
    genre_conversion_1_df = pd.read_csv('data/movie_adslots_conversion_1_3block.csv')
    genre_conversion_2_df = pd.read_csv('data/movie_adslots_conversion_2_3block.csv')
